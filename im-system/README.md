# SimpleIM - 轻量级即时通讯系统

一个基于 Go 语言实现的轻量级 IM 系统，支持千人同时在线，开箱即用。

[![Go](https://img.shields.io/badge/Go-1.21+-00ADD8?style=flat&logo=go)](https://golang.org)
[![License](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)

## ✨ 特性

- 🚀 **轻量高效** - 核心代码简洁，单节点支持万级连接
- 💬 **功能完整** - 单聊、群聊、离线消息、消息已读、消息撤回
- 📦 **开箱即用** - Docker 一键启动，自带 Web 演示页面
- 🔧 **易于扩展** - 清晰的分层架构，方便二次开发

## 🏗️ 架构

```
客户端 (Web/App)
      │
      │ WebSocket
      ▼
┌─────────────┐     ┌─────────────┐
│  Gateway 1  │────▶│  Gateway N  │  ← 无状态，可水平扩展
└──────┬──────┘     └──────┬──────┘
       │   Redis Pub/Sub   │
       └─────────┬─────────┘
                 ▼
    ┌────────────────────────┐
    │  MySQL │ Redis │ MinIO │
    └────────────────────────┘
```

## 🚀 快速开始

### 使用 Docker Compose（推荐）

```bash
# 克隆项目
git clone https://github.com/d60-lab/SimpleIM.git
cd SimpleIM/im-system

# 启动所有服务
make up

# 查看服务状态
make ps

# 访问演示页面
open http://localhost:8080
```

### 本地开发

```bash
# 仅启动依赖服务（MySQL、Redis、MinIO、MongoDB）
make deps

# 运行 Gateway 服务
make run

# 或者直接运行
go run cmd/gateway/main.go
```

### 常用命令

```bash
make help       # 查看所有可用命令
make up         # 启动所有服务
make down       # 停止所有服务
make logs       # 查看日志
make build      # 构建 Docker 镜像
make rebuild    # 重新构建并启动
make clean      # 清理所有容器和数据
make swagger    # 生成 Swagger 文档
```

## 📡 API 文档

### Swagger UI

启动服务后访问 Swagger 文档：

```
http://localhost:8080/swagger/index.html
```

支持在线测试所有 API 接口。

### 生成文档

```bash
# 安装 swag 工具（首次使用）
make swagger-install

# 生成/更新 Swagger 文档
make swagger
```

### API 接口

### 用户认证

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/register` | 用户注册 |
| POST | `/api/login` | 用户登录 |
| GET | `/api/user/info` | 获取用户信息 |
| PUT | `/api/user/info` | 更新用户信息 |
| GET | `/api/users/:id` | 根据ID获取用户 |
| GET | `/api/users` | 搜索用户 |

### 群组管理

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/groups` | 创建群组 |
| GET | `/api/groups/:id` | 获取群信息 |
| POST | `/api/groups/:id/join` | 加入群组 |
| POST | `/api/groups/:id/leave` | 退出群组 |
| GET | `/api/groups/:id/members` | 获取群成员 |
| GET | `/api/user/groups` | 获取我的群组 |

### 消息历史

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/messages/group/:id` | 获取群聊历史 |
| GET | `/api/messages/private/:id` | 获取私聊历史 |

### 文件上传

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/file/upload` | 上传文件 |
| GET | `/api/file/info/:id` | 获取文件信息 |
| GET | `/api/file/url/:id` | 获取文件URL |
| GET | `/api/file/download/:id` | 下载文件 |
| DELETE | `/api/file/:id` | 删除文件 |

### WebSocket

连接地址: `ws://localhost:8080/ws?token=<JWT_TOKEN>`

消息格式:
```json
{
  "type": 1,
  "to": "user_id",
  "content": { "text": "Hello!" },
  "timestamp": 1704067200000
}
```

消息类型:
| type | 说明 |
|------|------|
| 1 | 单聊消息 |
| 2 | 群聊消息 |
| 4 | 图片消息 |
| 7 | 文件消息 |
| 30 | 消息ACK |
| 99 | 心跳 |

## 📁 项目结构

```
im-system/
├── cmd/gateway/          # 服务入口
├── internal/
│   ├── gateway/          # 网关核心（连接管理、消息分发）
│   ├── service/          # 业务服务（群组、离线消息）
│   ├── handler/          # HTTP 接口
│   └── model/            # 数据模型
├── pkg/auth/             # JWT 认证
├── web/                  # 前端演示页面
├── deploy/               # 部署配置
│   ├── docker-compose.yml
│   └── Dockerfile
├── docs/                 # 文档
│   └── blog.md          # 技术博文
├── Makefile
└── README.md
```

## ⚙️ 配置

### 环境变量

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `NODE_ID` | node1 | 节点ID |
| `MYSQL_HOST` | localhost | MySQL 地址 |
| `MYSQL_PORT` | 3306 | MySQL 端口 |
| `MYSQL_USER` | root | MySQL 用户 |
| `MYSQL_PASSWORD` | password | MySQL 密码 |
| `MYSQL_DATABASE` | im_db | 数据库名 |
| `REDIS_HOST` | localhost | Redis 地址 |
| `REDIS_PORT` | 6379 | Redis 端口 |
| `JWT_SECRET` | im-secret | JWT 密钥 |

## 📊 性能

| 指标 | 数值 |
|------|------|
| 单节点连接数 | 10,000+ |
| 消息延迟 (P99) | < 50ms |
| 消息吞吐量 | 10,000+ QPS |

## 🛠️ 技术栈

- **语言**: Go 1.21+
- **Web框架**: Gin
- **WebSocket**: gorilla/websocket
- **数据库**: MySQL 8.0
- **缓存**: Redis 7
- **对象存储**: MinIO
- **容器化**: Docker

## 📖 文档

- [技术博文](docs/blog.md) - 详细的设计思路和实现说明

## 📄 License

MIT License